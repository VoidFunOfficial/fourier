...
export const SDK_ARTIFACT_SYMBOL = Symbol.for(SDK_ARTIFACT_SYMBOL_KEY);
export const SDK_ABI_VERSION = 1 as const;
export const SUPPORTED_SDK_ABI_VERSIONS = Object.freeze([SDK_ABI_VERSION] as const);
...
export function readSdkArtifact(
  ...
  if (metadata.sdkAbiVersion !== SDK_ABI_VERSION) {
    fail(
      "SDK_ABI_UNSUPPORTED",
      `不支持 SDK ABI ${String(metadata.sdkAbiVersion)}，当前支持 ${SDK_ABI_VERSION}`,
  ...
  if (
    ...
    fail(
      "ARTIFACT_EXPORT_INVALID",
      "SDK ABI v1 artifact 必须声明兼容 renderer component，且不能包含 render",
    );
  }
  if (
  ...
  if (
    ...
    fail(
      "ARTIFACT_EXPORT_INVALID",
      "SDK ABI v1 React artifact 的 static 必须是 boolean",
    );
  }
