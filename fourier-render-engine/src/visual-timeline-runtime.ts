...
function recoverableBrowserFailure(error: unknown): boolean {
  if (error instanceof RenderEngineError && error.code === "DOM_RUNTIME_TIMEOUT") return true;
...
async function terminateTimedOutExecution(
  ...
  if (!(error instanceof RenderEngineError) || error.code !== "DOM_RUNTIME_TIMEOUT") return;
  await withTimeout(
...
class BrowserPagePool {
  ...
  async close(): Promise<void> {
    ...
    this.#closed = true;
    const closedError = new RenderEngineError("TIMELINE_INSTANCE_CLOSED", "DOM page pool 已关闭");
...
export class DomTimelineAdapter implements TimelineAdapter {
  ...
  async open(artifact: CompiledVisualArtifact): Promise<TimelineInstance> {
    if (
      ...
      fail("ARTIFACT_RUNTIME_MISMATCH", "DomTimelineAdapter 只接受已编译的 ABI v1 artifact");
    }
      ...
      sample: async (request: TimelineSampleRequest): Promise<TimelineSampleResult> => {
        ...
        return cache.get(cacheKey, () => runExclusive(async () => {
          ...
          try {
            ...
            if (error instanceof RenderEngineError && error.code === "RENDER_CANCELLED") {
              state = "ready";
              throw error;
            }
...
export class VisualTimelineRuntime {
  ...
  constructor(options: { maximumDomPages?: number } = {}) {
