...
describeIntegration("Fourier World npm publish preparation", () => {
  test("逐个编译 npm 包成员并生成 MP4", async () => {
    const source = await Bun.file(new URL("fixtures/DomTestingPanel.tsx", import.meta.url)).text();
    const files = {
      "package/package.json": JSON.stringify({
        name: "@studio/components", version: "1.0.0", description: "collection", license: "MIT",
        fourier: { schemaVersion: 1, components: ["components/DomTestingPanel/package.json"] },
      }),
      ...
      "package/components/DomTestingPanel/main.tsx": source,
    };
    ...
    try {
      ...
      expect(new TextDecoder().decode(prepared.components[0]!.preview.bytes.subarray(4, 8))).toBe("ftyp");
    } finally {
