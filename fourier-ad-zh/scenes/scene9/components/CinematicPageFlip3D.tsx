...
import poster3Url from "../assets/poster3.png";
import poster4Url from "../assets/poster4.png";
import poster5Url from "../assets/poster5.png";
import poster6Url from "../assets/poster6.png";
import poster7Url from "../assets/poster7.png";
import poster8Url from "../assets/poster8.png";
import poster9Url from "../assets/poster9.png";
import poster10Url from "../assets/poster10.png";
import poster11Url from "../assets/poster11.png";
import {
...
const camera = defineCamera({
  ...
  moves: [{
    at: "0f",
    duration: `${SCENE_NINE_MONTAGE_FRAMES}f`,
...
export const cinematicPageFlip3DSchema = defineSchema({
  poster2: field.asset({
    ...
    default: poster2Url,
    label: "Poster 2",
    description: "The revised poster shown before the first Review swipe.",
  }),
  poster3: field.asset({
    accept: ["image/png", "image/jpeg", "image/webp"],
    default: poster3Url,
  ...
  poster11: field.asset({
    ...
    default: poster11Url,
    label: "Poster 11",
    description: "The final poster held after the fastest swipe.",
  }),
...
export function posterIndexAtTime(
  ...
  const lastIndex = Math.min(
    posterCount - 1,
    SCENE_NINE_POSTER_CUT_SECONDS.length - 1,
  );
  for (let index = lastIndex; index > 0; index -= 1) {
    if (safeTime >= SCENE_NINE_POSTER_CUT_SECONDS[index]!) return index;
  }
  return 0;
}
...
export function posterFlashPulse(
  ...
  const cutCount = Math.min(
    posterCount,
    SCENE_NINE_POSTER_CUT_SECONDS.length,
  );
...
function PosterMontageScene({
  ...
  return (
    <FourierCanvas
      ariaLabel="Accelerating cinematic poster page flip montage"
      ...
      onCreate={async ({ renderer, scene, camera: sceneCamera }) => {
        ...
        const textures = await new TextureLoader(undefined, {
          colorSpace: SRGBColorSpace,
        }).loadManyAsync(textureUrls);
      ...
      onFrame={({ timeSeconds, progress, camera: sceneCamera }) => {
        ...
        const activeStart = SCENE_NINE_POSTER_CUT_SECONDS[activeIndex] ?? 0;
        ...
        for (let index = 0; index < rig.posters.length; index += 1) {
          ...
          poster.group.rotation.y = (1 - settle) * (activeIndex % 2 === 0 ? 0.34 : -0.34);
          poster.group.rotation.z = (1 - settle) * (activeIndex % 2 === 0 ? -0.035 : 0.035);
