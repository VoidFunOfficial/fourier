  ...
  const visit = async (path: string): Promise<void> => {
    ...
    if (extension === ".css") {
      if (/url\(\s*["']?(?:https?:|\/\/)/i.test(source) || /@import\s+["'](?:https?:|\/\/)/i.test(source)) {
        fail("UNSUPPORTED_DOM_TIMELINE_API", "受支持 SDK ABI CSS 不允许网络 URL", { entryPath: path });
      }
      return;
    }
    ...
    for (const [pattern, label] of forbiddenDomSource) {
      if (pattern.test(source)) {
        fail(
          ...
          `受支持 SDK ABI artifact 不允许使用 ${label}`,
    ...
    for (const dependency of new Bun.Transpiler({ loader }).scanImports(source)) {
      if (dependency.path.startsWith(".")) {
        ...
        fail(
          ...
          `SDK ABI artifact 必须从 @fourier-video/sdk 导入 React 能力，禁止直接导入: ${dependency.path}`,
...
async function loadArtifact(
  ...
  if (options.exportName !== undefined && options.exportName !== "default") {
    fail("ARTIFACT_EXPORT_INVALID", "SDK ABI openArtifact 仅支持 default export");
  }
  ...
  try {
    const result = await Bun.build({
      ...
      plugins: [
        ...
        authorRuntimeAliasPlugin(
          "fourier-artifact-metadata-runtime",
...
export async function compileVisualArtifact(
  ...
  const loaded = await loadArtifact(options, integration);
  ...
  const bundleSnapshot = await browserBundle(loaded.entryPath, integration);
