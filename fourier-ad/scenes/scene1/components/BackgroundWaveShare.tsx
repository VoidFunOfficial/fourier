...
export const backgroundWaveShareSchema = defineSchema({
  ...
  revealStartSeconds: field.number({
    label: "Reveal start",
    description: "The local time when the centered circular opening begins.",
    min: 0,
    max: 60,
    default: 121 / 30,
  }),
  revealEndSeconds: field.number({
    label: "Reveal end",
    description: "The local time when the circular opening covers the canvas.",
    min: 0,
    max: 60,
    default: 163 / 30,
  }),
...
export function circularRevealFrames(
  width: number,
  height: number,
  revealStartSeconds: number,
  revealEndSeconds: number,
): readonly FourierMotionTarget[] {
  const duration = Math.max(0.001, revealEndSeconds);
  const startOffset = clamp(revealStartSeconds / duration, 0, 1);
  const radius = backgroundWaveMaximumRadius(width, height) + 64;
  const closed = "circle(0px at 50% 50%)";
  const open = `circle(${radius}px at 50% 50%)`;
  return [
    { clipPath: closed, offset: 0 },
    { clipPath: closed, offset: startOffset },
...
function BackgroundWaveShareLayer({ props }: { props: BackgroundWaveShareProps }) {
  ...
  return (
    <FourierMotion>
      <motion.div
        animate={circularRevealFrames(
          width,
          height,
          props.revealStartSeconds,
          props.revealEndSeconds,
        )}
        transition={{
          duration: Math.max(0.001, props.revealEndSeconds),
          ease: [0.4, 0, 0.2, 1],
          fill: "both",
        }}
        style={{
