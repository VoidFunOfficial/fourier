...
def write_vector(name, source, region, light=False):
    rgb = np.asarray(Image.open(ROOT/'reference/frames'/source).convert('RGB'))
    x,y,w,h = region
...
ledger = [
    write_vector('warrior.svg','0321.png',(330,132,410,475)),
...
subprocess.run(['ffmpeg','-hide_banner','-loglevel','error','-y','-i',str(ROOT/'clone.mp4'),'-vn','-ar','48000','-ac','2','-af','apad,atrim=duration=19.05',str(ROOT/'assets/audio/reference-master.wav')],check=True)
...
(ROOT/'reference/asset-sources.json').write_text(json.dumps(ledger,ensure_ascii=False,indent=2))
