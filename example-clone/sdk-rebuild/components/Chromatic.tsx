...
export function ColorBar({palette='all',seconds,frames,style={}}:{palette?:Palette;seconds:number;frames?:readonly FourierMotionTarget[];style?:CSSProperties}) {
  if(!frames) return <div style={{position:'absolute',height:110,width:1200,borderRadius:24,overflow:'hidden',...style}}><ChromaticFill palette={palette} seconds={seconds}/></div>;
  return <motion.div animate={frames} transition={{duration:seconds,ease:'linear',fill:'both'}} style={{position:'absolute',height:110,width:1200,borderRadius:24,overflow:'hidden',...style}}><ChromaticFill palette={palette} seconds={seconds}/></motion.div>;
}

export function Orb({name,color,size=216}:{name:IconName;color:string;size?:number}) {
  return <div style={{position:'relative',width:size,height:size}}>
    <div style={{position:'absolute',inset:0,maskImage:'radial-gradient(circle at 35% 35%,#000 0 42%,#000b 55%,#0000 75%)'}}><div style={{position:'absolute',inset:'3%',borderRadius:'50%',background:color,filter:'blur(5px)'}}/>
    ...
    </div><div style={{position:'absolute',left:'50%',top:'50%',transform:'translate(-50%,-50%)'}}><Icon name={name} size={size*.225} color="#fffdfd"/></div>
  </div>;
}
